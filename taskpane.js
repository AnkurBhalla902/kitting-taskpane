...
function applyFilters() {
  ...
  filteredRows = allRows.filter(row => {
    ...
    for (const col of Object.keys(dropVals)) {
      const selected = dropVals[col];
      const rowVal = String(row[col]??"");
      // For "Week To Raise Inbound", compare by canonical key so "5" matches "W05 (2026)"
      if (col === "Week To Raise Inbound") {
        const rowKey = canonicalWeekKey(rowVal);
        const selKey = canonicalWeekKey(selected);
        if (rowKey && selKey) { if (rowKey !== selKey) return false; }
        else { if (rowVal !== selected) return false; }
      } else {
        if (rowVal !== selected) return false;
      }
    }
    if (q) {
...
function populateFilterDropdowns() {
  FILTER_DROPDOWNS.forEach(f => {
    ...
    const seen = new Set();
    // Special handling: Week To Raise Inbound rolls up by canonical week+year
    const rollWeeks = (f.column === "Week To Raise Inbound");
    for (const row of allRows) {
      const v = row[f.column];
      if (v==null||v===""||String(v)==="#REF!") continue;
      let label = String(v);
      if (rollWeeks) {
        const key = canonicalWeekKey(label);
        if (key) label = displayWeekKey(key);
      }
      seen.add(label);
    }
...
function renderDashboard() {
  ...
  const weekInCountsRaw=countBy(active,"Week To Raise Inbound", v => v&&v!=="#REF!");
  // Roll up: "5" and "05_2026" → same key
  const weekInCounts = rollUpWeekCounts(weekInCountsRaw);
...
function sortCountsForWeek(counts) {
  // Sort by canonical (year, week) order
...
// Take a count map keyed by raw strings ("5","05_2026"...) and merge entries
// that resolve to the same canonical week+year, using the canonical key.
function rollUpWeekCounts(rawCounts) {
  const rolled = {};
  for (const [raw, n] of Object.entries(rawCounts)) {
    const key = canonicalWeekKey(raw);
    if (!key) {
      // Non-parseable values stay as-is
      rolled[raw] = (rolled[raw] || 0) + n;
    } else {
      const label = displayWeekKey(key);
...
function renderWeekView() {
  ...
  if (!allRows.length) { root.innerHTML="<p class='empty-state'>Load data first.</p>"; return; }
  const today=new Date(), cw=isoWeek(today);
  document.getElementById("week-current-label").textContent="Current week: W"+cw;
  document.getElementById("week-config-info").textContent="("+today.getFullYear()+")";
  ...
  // Default-open: overdue, thisweek, nextweek. Default-closed: later, noweek.
  // Auto-collapse any column with 0 rows.
  const cols = [
    { key:"overdue",  label:"Overdue", cls:"overdue", rows:buckets.overdue, defaultOpen:true },
    { key:"thisweek", label:"This week (W"+cw+")", cls:"thisweek", rows:buckets.thisweek, defaultOpen:true },
    { key:"nextweek", label:"Next week (W"+(cw+1)+")", cls:"nextweek", rows:buckets.nextweek, defaultOpen:true },
